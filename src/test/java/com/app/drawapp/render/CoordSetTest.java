...
import com.app.drawapp.fixture.ArrayProjection;
...
import static com.app.drawapp.render.CoordSet.row;
...
public class CoordSetTest {
    ...
    //test coord set 1:
    Point topFrameStart, topFrameEnd, leftFrameStart, leftFrameEnd, centerFrameStart, centerFrameEnd,
            rightFrameStart, rightFrameEnd, bottomFrameStart, bottomFrameEnd;

    //test coord set 2:
    Point topHalfStart, topHalfEnd, leftQuarterStart, leftQuarterEnd, rightQuarterStart, rightQuarterEnd;
    ...
    public void setDoesNotContainCoordsAfterRemoveAll(){
        coordSet = new CoordSet(100, 100);
        coordSet.addAllCoords();
        assertEquals(100*100, coordSet.size());
    ...
    public void filterReturnsCoordsWithinBoundary(){
        ...
        assertEquals((95-33+1)*(95-33+1), result.size());
    }
    ...
    public void addAllAddsAllNewElements(){
        ...
        forAllCoordsIn(topFrameStart, topFrameEnd, (set, point) -> assertFalse(set.contains(point.x, point.y)));
        forAllCoordsIn(bottomFrameStart, bottomFrameEnd, (set, point) -> assertFalse(set.contains(point.x, point.y)));
        assertEquals(8000, coordSet.size());
    }
    ...
    public void test2(){
        ...
        String retainedResultStr =
                ...
                "        "+
                "        ";
        CoordSet retain = createCoordSet(retainStr, 8, 8);
        CoordSet value = createCoordSet(valueStr, 8, 8);
        CoordSet retainedResult = createCoordSet(retainedResultStr, 8, 8);

        Long retainVal = retain.getAsLong(0,0);
        Long cellVal = value.getAsLong(0,0);
        Long resultVal = retainedResult.getAsLong(0,0);

        Long result = value.retain(retainVal, cellVal, 0, 1, -1L, 0L);
        assertEquals(resultVal, result);
    ...
    public void test4(){
        ...
        Long result = value.retain(retainVal, cellVal, 1, -1, -1L, 0L);
        assertEquals(resultVal, result);
